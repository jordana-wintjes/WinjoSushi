document.addEventListener('DOMContentLoaded', function () {
    ...
    function showModal(item, imgSrc) {
                            ...
                            <div class="container-fluid">
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <p class="item-removed">${item.uncheckedIngredients ? item.uncheckedIngredients : ''}</p>
                                        <p class="item-special">${item.specialRequests ? item.specialRequests : ''}</p>
        ...
        $('#itemModal').on('hidden.bs.modal', function () {
            ...
            // Reset customizations when the item modal is hidden
        ...
        document.getElementById('addItem').addEventListener('click', function () {
            ...
            // Logic to add the item with the specified quantity and customizations
            console.log('Item added:', {
                name: item.name,
                price: item.price,
                quantity: quantity,
                uncheckedIngredients: item.uncheckedIngredients,
    ...
    function showCustomizeModal(item) {
        var ingredientsCheckboxes = item.removableIngredients.map(ingredient => `
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="${ingredient}" id="${ingredient}" ${!item.uncheckedIngredients || !item.uncheckedIngredients.includes(ingredient) ? 'checked' : ''}>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="customSpecialRequests" rows="3">${item.specialRequests || ''}</textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal" id="cancelCustomizations">Cancel</button>
        ...
        document.getElementById('saveCustomizations').addEventListener('click', function () {
            ...
            // Update the item modal with customizations
            item.uncheckedIngredients = uncheckedIngredients.length > 0 ? `No ${uncheckedIngredients.join(', ')}` : '';
            item.specialRequests = specialRequests;

            // Close the customize modal
            $('#customizeModal').modal('hide');

            // Show the item modal again with updated content
            $('#itemModal').on('shown.bs.modal', function () {
                document.querySelector('.item-description').textContent = item.description;
                document.querySelector('.item-removed').textContent = item.uncheckedIngredients;
                document.querySelector('.item-special').textContent = item.specialRequests;
                document.querySelector('.item-price').textContent = item.price; // Update price if needed
            });

            $('#itemModal').modal('show');
        });

        document.getElementById('cancelCustomizations').addEventListener('click', function () {
